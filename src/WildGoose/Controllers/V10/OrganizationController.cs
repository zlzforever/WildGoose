...
public class OrganizationController(OrganizationService organizationService, IMemoryCache memoryCache) : ControllerBase
{
    ...
    [HttpGet("subList")]
    public Task<List<SubOrganizationDto>> GetSubLis([FromQuery] GetSubListQuery query)
    {
        return organizationService.GetSubListAsync(query);
    }

    [HttpGet("{id}/summary")]
    public Task<OrganizationSummaryDto> GetSummary([FromRoute] GetSummaryQuery query)
    {
