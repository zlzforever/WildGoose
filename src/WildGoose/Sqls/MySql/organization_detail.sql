-- 创建数据主表
create table if not exists ${table_prefix}organization_detail
(
...
create index ${table_prefix}organization_detail_path_index
    on ${table_prefix}organization_detail (path);
create index ${table_prefix}organization_detail_parent_id_index
    on ${table_prefix}organization_detail (parent_id);

-- 创建数据副表
create table if not exists ${table_prefix}organization_detail_tmp
(
    id                     varchar(36)   not null,
    name                   varchar(255)  null,
    code                   varchar(255)  null,
...
create index ${table_prefix}organization_detail_path_index
    on ${table_prefix}organization_detail_tmp (path);
create index ${table_prefix}organization_detail_parent_id_index
    on ${table_prefix}organization_detail_tmp (parent_id);

-- 创建更新存储过程
CREATE PROCEDURE if not exists refresh_organization_detail()
BEGIN
-- 步骤1：清空备用表（此时主表仍对外提供数据，无影响）
TRUNCATE TABLE `${table_prefix}organization_detail_tmp`;
-- 步骤2：将新数据写入备用表
insert into `${table_prefix}organization_detail_tmp`
WITH
...
RENAME TABLE
        ${table_prefix}organization_detail TO z_${table_prefix}organization_detail_old,
        ${table_prefix}organization_detail_tmp TO ${table_prefix}organization_detail,
        z_${table_prefix}organization_detail_old TO ${table_prefix}organization_detail_tmp;
-- 步骤4：清空备用表
TRUNCATE TABLE ${table_prefix}organization_detail_tmp;
END;
